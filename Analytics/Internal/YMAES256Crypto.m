...
#import "YMAES256Crypto.h"
#import "YMUtils.h"

// Implementation courtesy of http://robnapier.net/aes-commoncrypto

static NSString *const kRNCryptManagerErrorDomain = @"com.yuktamedia.crypto";
...
{
    NSData *iv = [YMAES256Crypto randomDataOfLength:kAlgorithmIVSize];
    NSData *salt = [YMAES256Crypto randomDataOfLength:kPBKDFSaltSize];
...
- (NSData *)encrypt:(NSData *)data
{
    ...
    if (result == kCCSuccess) {
        ...
        NSError *error = [NSError errorWithDomain:kRNCryptManagerErrorDomain
                                             code:result
                                         userInfo:nil];
        YMLog(@"Unable to encrypt data", error);
        return nil;
    }
    return cipherData;
}
...
- (NSData *)decrypt:(NSData *)data
{
    ...
    if (result == kCCSuccess) {
        ...
        NSError *error = [NSError errorWithDomain:kRNCryptManagerErrorDomain
                                             code:result
                                         userInfo:nil];
        YMLog(@"Unable to decrypt data", error);
        return nil;
    }
...
+ (NSData *)randomDataOfLength:(size_t)length
{
    ...
    int result = SecRandomCopyBytes(kSecRandomDefault,
                                    ...
                                    data.mutableBytes);
    if (result != kCCSuccess) {
        YMLog(@"Unable to generate random bytes: %d", result);
    }
                         ...
                         salt:(NSData *)salt
{
    ...
    if (result != kCCSuccess) {
        YMLog(@"Unable to create AES key for password: %d", result);
    }
