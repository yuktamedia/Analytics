...
@implementation YMFileStorage

- (instancetype)initWithFolder:(NSURL *)folderURL crypto:(id<YMCrypto>)crypto
{
    if (self = [super init]) {
...
- (void)removeKey:(NSString *)key
{
    ...
    if (![[NSFileManager defaultManager] removeItemAtURL:url error:&error]) {
        YMLog(@"Unable to remove key %@ - error removing file at path %@", key, url);
    }
}
...
- (void)resetAll
{
    ...
    if (![[NSFileManager defaultManager] removeItemAtURL:self.folderURL error:&error]) {
        YMLog(@"ERROR: Unable to reset file storage. Path cannot be removed - %@", self.folderURL.path);
    }
...
- (void)setData:(NSData *)data forKey:(NSString *)key
{
    ...
    if (![url setResourceValue:@YES
                        forKey:NSURLIsExcludedFromBackupKey
                         error:&error]) {
        YMLog(@"Error excluding %@ from backup %@", [url lastPathComponent], error);
    }
}
...
- (NSData *)dataForKey:(NSString *)key
{
    ...
    if (!data) {
        YMLog(@"WARNING: No data file for key %@", key);
        return nil;
    }
    if (self.crypto) {
    ...
    if (data) {
        ...
        if (needsConversion) {
            @try {
                ...
                YMLog(@"Unable to convert data from plist object to json; Exception: %@, data: %@", e, data);
...
- (NSData *_Nullable)dataFromJSON:(id)json
{
    ...
    if (error) {
        YMLog(@"Unable to serialize data from json object; %@, %@", error, json);
    }
    return data;
}
    ...
    if (error) {
        ...
        if (result != nil) {
            *needsConversion = YES;
            ...
            YMLog(@"Unable to parse json from data %@", error);
        }
    }
    return result;
}
    ...
    if (![[NSFileManager defaultManager] fileExistsAtPath:url.path
        ...
        if (![[NSFileManager defaultManager] createDirectoryAtPath:url.path
                                       withIntermediateDirectories:YES
                                                        attributes:nil
                                                             error:&error]) {
            YMLog(@"error: %@", error.localizedDescription);
        }
    }
}
...
- (NSData *_Nullable)dataFromPlist:(nonnull id)plist
{
    ...
    @try {
        ...
        YMLog(@"Unable to serialize data from plist object; Exception: %@, plist: %@", e, plist);
    } @finally {
        if (error) {
            YMLog(@"Unable to serialize data from plist object; Error: %@, plist: %@", error, plist);
        }
    }
    return data;
}
