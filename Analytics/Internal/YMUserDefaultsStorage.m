...
@implementation YMUserDefaultsStorage

- (instancetype)initWithDefaults:(NSUserDefaults *)defaults namespacePrefix:(NSString *)namespacePrefix crypto:(id<YMCrypto>)crypto
{
    if (self = [super init]) {
        _defaults = defaults;
...
- (id _Nullable)plistForKey:(NSString *)key
{
    ...
    return data ? [YMUtils plistFromData:data] : nil;
}

- (void)setPlist:(id _Nonnull)plist forKey:(NSString *)key
{
    NSData *data = [YMUtils dataFromPlist:plist];
    if (data) {
