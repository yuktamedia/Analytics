...
//NSString *const YMADClientClass = @"ADClient";
...
+ (NSData *_Nullable)dataFromPlist:(nonnull id)plist
{
    ...
    NSData *data = [NSPropertyListSerialization dataWithPropertyList:plist
                                                             ...
                                                             options:0
                                                               error:&error];
    if (error) {
        YMLog(@"Unable to serialize data from plist object", error, plist);
    }
    return data;
}
    ...
    id plist = [NSPropertyListSerialization propertyListWithData:data
                                                         options:0
                                                          format:nil
                                                           error:&error];
    if (error) {
        YMLog(@"Unable to parse plist from data %@", error);
    }
    return plist;
}
...
+(id)traverseJSON:(id)object andReplaceWithFilters:(NSDictionary<NSString*, NSString*>*)patterns
{
    ...
    if ([object isKindOfClass:NSString.class]) {
        ...
        for (NSString* pattern in patterns) {
            ...
            if (matches > 0) {
                YMLog(@"%@ Redacted value from action: %@", self, pattern);
            }
        }
...
NSDictionary *getStaticContext(YMAnalyticsConfiguration *configuration, NSString *deviceToken)
{
    NSMutableDictionary *dict = [[NSMutableDictionary alloc] init];

    dict[@"library"] = @{
        @"name" : @"analytics-ios",
        @"version" : [YMAnalytics version]
    };
...
#if TARGET_OS_IPHONE
NSDictionary *mobileSpecifications(YMAnalyticsConfiguration *configuration, NSString *deviceToken)
{
    NSMutableDictionary *dict = [[NSMutableDictionary alloc] init];
    ...
    /*#if !(TARGET_IPHONE_SIMULATOR)
        Class adClient = NSClassFromString(YMADClientClass);
        if (adClient) {
...
NSDictionary *desktopSpecifications(YMAnalyticsConfiguration *configuration, NSString *deviceToken)
{
...
NSDictionary *getLiveContext(YMReachability *reachability, NSDictionary *referrer, NSDictionary *traits)
{
...
@interface YMISO8601NanosecondDateFormatter: NSDateFormatter
@end

@implementation YMISO8601NanosecondDateFormatter
...
const NSInteger __YM_NANO_MAX_LENGTH = 9;
- (NSString * _Nonnull)stringFromDate:(NSDate *)date
{
    ...
    if (nanoSeconds.length > __YM_NANO_MAX_LENGTH) {
        nanoSeconds = [nanoSeconds substringToIndex:__YM_NANO_MAX_LENGTH];
    } else {
        nanoSeconds = [nanoSeconds stringByPaddingToLength:__YM_NANO_MAX_LENGTH withString:@"0" startingAtIndex:0];
    }
...
NSString *iso8601NanoFormattedString(NSDate *date)
{
    ...
    dispatch_once(&onceToken, ^{
        dateFormatter = [[YMISO8601NanosecondDateFormatter alloc] init];
    });
...
// Async Utils
dispatch_queue_t
ym_dispatch_queue_create_specific(const char *label,
...
{
    ym_dispatch_specific(queue, block, NO);
}

void ym_dispatch_specific_sync(dispatch_queue_t queue,
                                dispatch_block_t block)
{
    ym_dispatch_specific(queue, block, YES);
}

// JSON Utils

static id YMCoerceJSONObject(id obj)
{
    ...
    if ([obj isKindOfClass:[NSArray class]]) {
        ...
        for (id i in obj) {
            ...
            [array addObject:YMCoerceJSONObject(value)];
        }
        return array;
    }
    ...
    if ([obj isKindOfClass:[NSDictionary class]]) {
        ...
        for (NSString *key in obj) {
            ...
            if (![key isKindOfClass:[NSString class]])
                YMLog(@"warning: dictionary keys should be strings. got: %@. coercing "
                       @"to: %@",
                       [key class], [key description]);
            dict[key.description] = YMCoerceJSONObject(value);
        }
        return dict;
    }
    ...
    YMLog(@"warning: dictionary values should be valid json types. got: %@. "
           @"coercing to: %@",
           [obj class], [obj description]);
    return [obj description];
}

NSDictionary *YMCoerceDictionary(NSDictionary *dict)
{
    ...
    return YMCoerceJSONObject(dict);
}
...
#else
            YMLog(@"key `%@` doesn't conform to NSCoding and can't be serialized for delivery.", key);
            // simply leave it out since we can't encode it anyway.
            continue;
#endif
        }
        
        if ([aValue conformsToProtocol:@protocol(YMSerializableDeepCopy)]) {
...
#else
            YMLog(@"type `%@` doesn't conform to NSCoding and can't be serialized for delivery.", NSStringFromClass([aValue class]));
            // simply leave it out since we can't encode it anyway.
            continue;
#endif
        }
