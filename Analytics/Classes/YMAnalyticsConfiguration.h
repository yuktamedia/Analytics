...
@protocol YMApplicationProtocol <NSObject>

#if TARGET_OS_IPHONE
@property (nullable, nonatomic, assign) id<UIApplicationDelegate> delegate;
- (NSUInteger)ym_beginBackgroundTaskWithName:(nullable NSString *)taskName expirationHandler:(void (^__nullable)(void))handler;
- (void)ym_endBackgroundTask:(NSUInteger)identifier;
#elif TARGET_OS_OSX
...
#if TARGET_OS_IOS
@interface UIApplication (YMApplicationProtocol) <YMApplicationProtocol>
@end
#elif TARGET_OS_OSX
@interface NSApplication (YMApplicationProtocol) <YMApplicationProtocol>
@end
#endif


typedef NSMutableURLRequest *_Nonnull (^YMRequestFactory)(NSURL *_Nonnull);
typedef NSString *_Nonnull (^YMAdSupportBlock)(void);

@protocol YMIntegrationFactory;
@protocol YMCrypto;
@protocol YMMiddleware;

@class YMAnalyticsExperimental;
@class YMDestinationMiddleware;
...
@interface YMAnalyticsConfiguration : NSObject
 ...
 */
@property (nonatomic, strong, nullable) YMRequestFactory requestFactory;
 ...
 */
@property (nonatomic, strong, nullable) NSArray<id<YMMiddleware>> *middlewares DEPRECATED_MSG_ATTRIBUTE("Use .sourceMiddleware instead.");

/**
 * Set custom source middleware. Will be run before all integrations
 */
@property (nonatomic, strong, nullable) NSArray<id<YMMiddleware>> *sourceMiddleware;

/**
 * Set custom destination middleware. Will be run before the associated integration for a destination.
 */
@property (nonatomic, strong, nullable) NSArray<YMDestinationMiddleware *> *destinationMiddleware;

/**
 * Register a factory that can be used to create an integration.
 */
- (void)use:(id<YMIntegrationFactory> _Nonnull)factory;

/**
 * Leave this nil for iOS extensions, otherwise set to UIApplication.sharedApplication.
 */
@property (nonatomic, strong, nullable) id<YMApplicationProtocol> application;
 ...
 *      }
 */
@property (nonatomic, strong, nullable) YMAdSupportBlock adSupportBlock;

/**
 Enable experimental features within the Segment Analytics-iOS library.
 */
@property (nonatomic, readonly, nonnull) YMAnalyticsExperimental *experimental;

@end

#pragma mark - Experimental

typedef  NSDictionary * _Nonnull (^YMRawModificationBlock)( NSDictionary * _Nonnull rawPayload);

NS_SWIFT_NAME(AnalyticsExperimental)
@interface YMAnalyticsExperimental : NSObject
/**
 ...
 */
@property (nonatomic, strong, nullable) YMRawModificationBlock rawAnalyticsModificationBlock;
